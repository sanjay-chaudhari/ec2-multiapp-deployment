...
```bash
# Delete existing key pair if it already exists, then recreate
aws ec2 delete-key-pair --key-name MyAppKeyPair --region us-east-1 2>/dev/null || true
...
your_ip       = "x.x.x.x/32"             # your PUBLIC IPv4: curl -4 ifconfig.me  (use -4 to force IPv4)
```

> Note: `curl ifconfig.me` may return an IPv6 address on some networks. Always use `curl -4 ifconfig.me` to get your IPv4.

Find the correct Ubuntu 22.04 AMI for your region and set it in `terraform.tfvars`:
```bash
...
```
> Change `--region` if you're not deploying to `us-east-1`.
...
Copy the script and run it on the App EC2:

```bash
scp -i MyAppKeyPair.pem scripts/setup_app_ec2.sh ubuntu@<APP_EC2_PUBLIC_IP>:~/
ssh -i MyAppKeyPair.pem ubuntu@<APP_EC2_PUBLIC_IP>
chmod +x ~/setup_app_ec2.sh
~/setup_app_ec2.sh myapp https://github.com/<your-org>/<your-repo>.git 8000
```

> Replace `<your-org>/<your-repo>` with your actual GitHub repo path. The script installs all dependencies (Python, Node 18, Nginx), clones the repo, builds the frontend, and configures systemd + Nginx automatically.

After the script completes, set your DB credentials and start the service:
```bash
