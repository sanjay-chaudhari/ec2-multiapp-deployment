...
# Install Node.js 18 via NodeSource (apt ships v12 which is too old for react-scripts)
if ! node --version 2>/dev/null | grep -q "^v1[89]\|^v[2-9]"; then
  sudo dpkg --remove --force-depends libnode-dev libnode72 2>/dev/null || true
